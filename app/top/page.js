  ...
  return (
    <>
      <div className="text-center py-6">
        <h1 style={{ fontFamily: "Dosis", fontWeight: 400, fontSize: "40px" }}>
          Top Sellers
        </h1>
      </div>
      <div className="grid justify-center items-center mx-12 space-x-3 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {Array.isArray(data) ? (
          data.map((item) => (
            <div
              key={item._id}
              className="w-60 h-[27rem] border-slate-200 border m-2"
            >
              <Image
              ...
              />
              <div className="h-6 py-8 ml-2 text-lg truncate">{item.name}</div>
              <div
                style={{
                  fontFamily: "Dosis",
                  fontWeight: 200,
              ...
              <div>
                <Link href={`/fproduct/${item._id}`}>
                  <button
                    ...
                    className="border hover:bg-black hover:text-white px-10 py-1 mb-2 ml-2"
                  >
                    View
                  </button>
                </Link>
